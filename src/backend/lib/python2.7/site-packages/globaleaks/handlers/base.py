...
def remove_metadata_photo(file):
        """
        Returns file without metadata
        """
        img_file = open(file)
        img = Image.open(img_file)
        data = list(img.getdata())
        img_clear = Image.new(img.mode, img.size)
        img_clear.putdata(data)
        img_clear.save('%s' % file)
...
class BaseHandler(RequestHandler):
    ...
    def write_file(self, filepath, rfile=None):
        try:
            new = ""
            if rfile:
                ext = rfile['name'].split('.')[-1]
                new = filepath+"."+ext
                old = filepath
                shutil.copyfile(filepath, new)
                if ext == "jpeg" or ext == "jpg":
                    log.err("Removing metadata...")
                    remove_metadata_photo(new)
                    log.err("Metadata removed!")
                    filepath = new
                elif ext == "docx" or ext == "pdf":
                    log.err("Removing document metadata...")
                    remove_metadata_text(new, ext)
                    log.err("Metadata removed!")
                    filepath = new
                else:
                    filepath = old
