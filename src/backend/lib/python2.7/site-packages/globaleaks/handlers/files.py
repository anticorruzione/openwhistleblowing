...
def serialize_file(internalfile):
    return {
        ...
        'href': "/wbtip/download/" + internalfile.id,
        'path': internalfile.file_path,
        'wb_download_date': internalfile.wb_download_date
    }
...
@transact_ro
def get_itip_id_by_rtip_id(store, rtip_id):
    rtip = store.find(ReceiverTip,
                      ReceiverTip.id == rtip_id).one()

    if not rtip:
        raise errors.TipIdNotFound

    return rtip.internaltip.id
...
class FileAdd(BaseHandler):
    ...
    @inlineCallbacks
    def handle_file_append(self, itip_id, description=None):
        ...
        uploaded_file['wb_download_date'] = datetime_now()
        uploaded_file['submission'] = False
        if description:
            uploaded_file['description'] = description
        uploaded_file['is_visible'] = True
    ...
    def post(self, *args):
        ...
        if len(args) > 0:
            itip_id = yield get_itip_id_by_rtip_id(args[0])
            description = None
            if 'description' in self.request.arguments:
                description = self.request.arguments['description'][0]
            yield self.handle_file_append(itip_id, description)
        else:
...
# This is different from FileInstance, just because there are a different authentication requirements
class RTipFileAdd(BaseHandler):
    """
    Receiver interface for upload a new file in an already completed submission
    """
    handler_exec_time_threshold = 3600
    filehandler = True
    ...
    def handle_file_append(self, itip_id, description=None):
        ...
        if description:
            uploaded_file['description'] = description
        uploaded_file['is_visible'] = False
...
@transact
def download_file_wb(store, file_id):
    """
    Auth temporary disabled, just Tip_id and File_id required
    """
    ifile = store.find(InternalFile,
                       InternalFile.id == unicode(file_id)).one()

    if not ifile:
        raise errors.FileIdNotFound

    if not ifile.wb_download_date:
        ifile.wb_download_date = datetime_now()
...
class DownloadWB(BaseHandler):
    handler_exec_time_threshold = 3600

    @unauthenticated
    @inlineCallbacks
    def post(self, ifile_id):
        ifile = yield download_file_wb(ifile_id)

        filelocation = os.path.join(GLSettings.submission_path, ifile['path'])

        if os.path.exists(filelocation):
            self.set_status(200)
            self.set_header('X-Download-Options', 'noopen')
            self.set_header('Content-Type', 'application/octet-stream')
            self.set_header('Content-Disposition', 'attachment; filename=\"%s\"' % ifile['name'])
            ris = self.write_file(filelocation, ifile)
            if ris is not None:
                self.set_header('Content-Length', ris)
            else:
                self.set_header('Content-Length', ifile['size'])
        else:
            self.set_status(404)
