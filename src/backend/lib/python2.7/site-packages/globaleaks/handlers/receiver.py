...
from globaleaks.models import Receiver, ReceiverTip, Custodian, IdentityAccessRequest
...
def get_receiver_settings(store, receiver_id, language):
    ...
    if not user:
        user = store.find(Receiver, Receiver.id == receiver_id).one()

    if not user:
        raise errors.ReceiverIdNotFound

    return receiver_serialize_receiver(user, language)
