...
from globaleaks.models import Notification, Comment, Message, \
    ...
    SecureFileDelete, IdentityAccessRequest, Custodian
...
def db_access_rtip(store, user_id, rtip_id):
    user = store.find(Custodian, Custodian.id == user_id).one()
    ...
    if not user:
        ...
        rtip = store.find(ReceiverTip, ReceiverTip.id == unicode(rtip_id)).one()
        iar = store.find(IdentityAccessRequest, IdentityAccessRequest.receivertip_id == rtip.id).one()
        if iar.reply == u'pending':
            return rtip


def db_get_files_receiver(store, user_id, rtip_id, serialize=True):
...
def db_get_rtip_status(store, rtip):
    return [{
        "status_id": rtip.internaltip.status_id
    }]
...
@transact_ro
def get_internaltip_status(store, user_id, rtip_id):
    rtip = db_access_rtip(store, user_id, rtip_id)

    return db_get_rtip_status(store, rtip)
...
@transact
def set_internaltip_status(store, user_id, rtip_id, status):
    rtip = db_access_rtip(store, user_id, rtip_id)

    db_update_status(rtip, status)


@transact
def set_internaltip_receiver(store, user_id, rtip_id, receiver):
    rtip = db_access_rtip(store, user_id, rtip_id)
    receiver_files = db_get_files_receiver(store, user_id, rtip_id, False)

    db_update_receiver(rtip, receiver, receiver_files)
...
class RTipStatusHandler(BaseHandler):
    """
    ...
    def get(self, tip_id):
        ...
        Errors: InvalidAuthentication
        """
        rtip_status = yield get_internaltip_status(self.current_user.user_id, tip_id)

        self.set_status(200)
        self.finish(rtip_status)
    ...
    def post(self, tip_id):
        ...
        # getting the status_id from the JSON
        import json
        status = json.loads(self.request.body)

        yield set_internaltip_status(self.current_user.user_id, tip_id, status['status_id'])

        self.set_status(200)
        self.finish()


class RTipReceiverHandler(BaseHandler):
    """
    This interface return the summary list of the Tips available for the authenticated Receiver
    GET /receiver
    """
    ...
    def get(self, tip_id):
        ...
        Errors: InvalidAuthentication
        """
        rtip_receiver = yield get_internaltip_receiver(self.current_user.user_id, tip_id)

        self.set_receiver(200)
        self.finish(rtip_receiver)
    ...
    def post(self, tip_id):
        ...
        # getting the receiver_id from the JSON
        import json
        receiver = json.loads(self.request.body)

        yield set_internaltip_receiver(self.current_user.user_id, tip_id, receiver['receiver_id'])
...
class RTipReceiversCollection(BaseHandler):
    ...
    def get(self, tip_id):
        ...
        Errors: InvalidAuthentication
        """
        answer = yield get_receiver_list(self.current_user.user_id, tip_id, self.request.language)
